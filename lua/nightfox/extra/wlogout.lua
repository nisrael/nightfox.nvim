local template = require("nightfox.util.template")

local M = {}

function M.generate(spec, _)
  local content = [[
/**
 * Nightfox theme for wlogout
 * name: ${palette.meta.name}
 * upstream: ${palette.meta.url}
 */

* {
    background-image: none;
}

window {
    font-family: "JetBrains Mono", "Fira Code", monospace, sans-serif;
    font-size: 14pt;
    font-weight: 500;
    color: ${fg2}; /* subtext */
    background-color: ${bg0}f2; /* mantle with opacity */
}

button {
    background-repeat: no-repeat;
    background-position: center 35%;
    background-size: 25%;
    border: 2px solid ${bg3}; /* overlay0 */
    border-radius: 12px;
    background-color: ${bg2}; /* surface1 */
    margin: 5px;
    padding: 20px;
    min-width: 160px;
    min-height: 80px;
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3);
    text-shadow: none;
    -gtk-icon-shadow: none;
    transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1);
}

button:hover {
    background-color: ${sel0}; /* surface2 */
    border-color: ${bg4}; /* overlay1 */
    color: ${fg1}; /* text */
    box-shadow: 0 6px 12px rgba(0, 0, 0, 0.4);
}

button:focus {
    background-color: ${sel0};
    color: ${fg1};
}

/* Individual button accent colors on hover */
#lock:hover {
    border-color: ${palette.blue}; /* blue */
}

#lock:focus {
    border-color: ${palette.blue};
}

#logout:hover {
    border-color: ${palette.yellow}; /* yellow */
}

#logout:focus {
    border-color: ${palette.yellow};
}

#suspend:hover {
    border-color: ${palette.magenta}; /* magenta */
}

#suspend:focus {
    border-color: ${palette.magenta};
}

#reboot:hover {
    border-color: ${palette.orange}; /* orange */
}

#reboot:focus {
    border-color: ${palette.orange};
}

#shutdown:hover {
    border-color: ${palette.red}; /* red */
}

#shutdown:focus {
    border-color: ${palette.red};
}

/* Button images - user should provide their own */
#lock {
    background-image: image(url("./lock.png"));
}
#lock:hover,
#lock:focus {
    background-image: image(url("./lock-hover.png"));
}

#logout {
    background-image: image(url("./logout.png"));
}
#logout:hover,
#logout:focus {
    background-image: image(url("./logout-hover.png"));
}

#suspend {
    background-image: image(url("./suspend.png"));
}
#suspend:hover,
#suspend:focus {
    background-image: image(url("./suspend-hover.png"));
}

#reboot {
    background-image: image(url("./reboot.png"));
}
#reboot:hover,
#reboot:focus {
    background-image: image(url("./reboot-hover.png"));
}

#shutdown {
    background-image: image(url("./shutdown.png"));
}
#shutdown:hover,
#shutdown:focus {
    background-image: image(url("./shutdown-hover.png"));
}
]]

  return template.parse_template_str(content, spec)
end

return M
